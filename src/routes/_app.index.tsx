...
import { FileText, Clock, DollarSign, ReceiptText, Plus, UserPlus, GitCompareArrows } from "lucide-react";
...
function KPI({ icon: Icon, label, value, tone }: { icon: typeof FileText; label: string; value: string; tone: string }) {
  return (
    <Card className="p-5 flex items-start gap-4 hover:shadow-md transition-shadow">
      <div className={`size-11 rounded-xl grid place-items-center shrink-0 ${tone}`}><Icon className="size-5" /></div>
      <div>
  ...
  return (
    <>
      ...
      <main className="p-6 space-y-6">
        <div>
          <h2 className="text-xl font-bold">Welcome back, {currentProfile?.firstName || role} 👋</h2>
          <p className="text-sm text-muted-foreground mt-0.5">Here's your procurement overview for today</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KPI icon={ReceiptText} label="Total Purchase Orders" value={String(totalInvoices)} tone="bg-primary/10 text-primary" />
          <KPI icon={UserPlus} label="Active Vendors" value={String(activeVendors)} tone="bg-success/10 text-success" />
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <Card className="p-5 space-y-2">
            ...
            <Button asChild variant="outline" className="w-full justify-start"><Link to="/quotation-comparison"><GitCompareArrows className="size-4" /> Compare Quotations</Link></Button>
                ...
                {documents.slice(0, 5).map((d) => {
                  ...
                  return (
                    <tr key={d.id} className="border-b last:border-b-0 hover:bg-muted/20 transition-colors">
                      <td className="py-2.5 font-mono text-xs font-semibold text-primary">{d.poNumber}</td>
                      <td className="font-medium">{v?.name || "—"}</td>
                      <td className="font-semibold">
                        {total > 0 ? `₹${total.toLocaleString("en-IN", { maximumFractionDigits: 0 })}` : "—"}
                      </td>
                      <td>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${d.status === "Paid" ? "bg-success/15 text-success" : "bg-warning/15 text-warning-foreground"}`}>
                          {d.status}
                        </span>
                      </td>
                    </tr>
