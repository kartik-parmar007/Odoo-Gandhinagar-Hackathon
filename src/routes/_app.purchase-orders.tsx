            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded-full ${d.status === "Paid" ? "bg-success/15 text-success" : "bg-warning/15 text-warning-foreground"}`}
                  >
                    {d.status}
                  </span>
                </div>
            ...
            <>
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Auto-generated after approval · {doc.date}
                  </p>
                </div>
                <div className="flex gap-2 flex-wrap">
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      toast.success(`Invoice PDF for ${doc.poNumber} downloaded successfully!`)
                    }
                  >
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => toast.success(`Document ${doc.poNumber} sent to printer`)}
                  >
                    <Printer className="size-4" /> Print
                  </Button>
                  <Button
                    size="sm"
                    onClick={() => toast.success(`PO ${doc.poNumber} emailed to ${v?.name}`)}
                  >
              ...
              <Card className="p-8 max-w-4xl space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm bg-muted/20 p-4 rounded-lg">
                  <div>
                    <span className="text-muted-foreground block text-xs uppercase">PO Number</span>{" "}
                    <span className="font-mono font-semibold">{doc.poNumber}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="font-semibold">{rfq?.title || "N/A"}</span>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t flex justify-between items-center flex-wrap gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${doc.status === "Paid" ? "bg-success/15 text-success" : "bg-warning/15 text-warning-foreground"}`}
                    >
                      {doc.status}
                    </span>
                  </div>
                      ...
                      {canMarkAsPaid ? (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            log({
                              type: "po",
                              message: `PO ${doc.poNumber} marked as PAID by ${role}`,
                            });
                        ...
                        <span className="text-xs text-muted-foreground italic">
                          Only Procurement Officers/Managers/Admins can record payments
                        </span>
                      )}
                    </>
                  )}
