        ...
        <Tabs defaultValue={isVendor ? "submit" : "compare"}>
          <TabsList>
            ...
            <TabsTrigger value="submit">
              {isVendor ? "Submit Bid" : "Submit Quotation (Demo)"}
            </TabsTrigger>
          </TabsList>
          {!isVendor && (
...
function SubmitView() {
  ...
  const [lines, setLines] = useState(
    rfq?.items.map((i) => ({ item: i.name, qty: i.qty, unitPrice: 0 })) ?? [],
  );
  ...
  const submit = () => {
    ...
    if (lines.some((l) => l.unitPrice <= 0))
      return toast.error("Please enter a unit price for all items");

    addQuotation({ rfqId, vendorId, lines, deliveryDays: days, notes });
    const activeVendor = vendors.find((v) => v.id === vendorId);
    log({
      type: "quotation",
      message: `Quotation submitted by ${activeVendor?.name || "Vendor"} for ${rfq?.title}`,
    });
      ...
      {rfq && (
        <div className="bg-muted/40 p-3 rounded-md text-sm">
          ...
          <div className="text-muted-foreground text-xs mt-1">
            {rfq.items.map((i) => `${i.name} ×${i.qty}`).join(" · ")} — {rfq.category}
          </div>
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="bg-muted/30 p-3 rounded-md space-y-1 text-sm">
          ...
          <div className="flex justify-between font-bold pt-1 border-t">
            ...
            <span>₹{(total + gst).toLocaleString()}</span>
          </div>
        </div>
      </div>
...
function CompareView() {
  ...
  const sendApproval = () => {
    ...
    log({
      type: "approval",
      message: `Quotation sent to manager approval for ${rfq?.title} (Lowest Bidder: ${v?.name || "Vendor"})`,
    });
      ...
      {quotes.length === 0 ? (
        ...
        <>
                ...
                <tr className="bg-muted/40 text-xs">
                  ...
                  {quotes.map((q, i) => {
                    ...
                    return (
                      <th
                        key={q.id}
                        className={`text-left p-3 transition-colors ${winner ? "bg-emerald-50/80 dark:bg-emerald-950/20 text-emerald-900 dark:text-emerald-300 border-l-2 border-r-2 border-t-2 border-emerald-500" : ""}`}
                      >
              ...
              <tbody>
                <tr className="border-t">
                  ...
                  {quotes.map((q, i) => (
                    <td
                      key={q.id}
                      className={`p-3 font-bold ${i === lowestIdx ? "bg-emerald-50/80 dark:bg-emerald-950/20 text-emerald-900 dark:text-emerald-300 border-l-2 border-r-2 border-emerald-500" : ""}`}
                    >
                ...
                <tr className="border-t">
                  ...
                  {quotes.map((q, i) => (
                    <td
                      key={q.id}
                      className={`p-3 ${i === lowestIdx ? "bg-emerald-50/80 dark:bg-emerald-950/20 text-emerald-900 dark:text-emerald-300 border-l-2 border-r-2 border-emerald-500" : ""}`}
                    >
                      {q.deliveryDays} days
                    </td>
                  ))}
                ...
                <tr className="border-t">
                  ...
                  {quotes.map((q, i) => {
                    ...
                    return (
                      <td
                        key={q.id}
                        className={`p-3 font-mono text-xs ${i === lowestIdx ? "bg-emerald-50/80 dark:bg-emerald-950/20 text-emerald-900 dark:text-emerald-300 border-l-2 border-r-2 border-b-2 border-emerald-500" : ""}`}
                      >
                        {v?.gst}
                      </td>
                    );
          ...
          <p className="text-xs text-muted-foreground flex items-center gap-1">
            ...
            <span>
              Lowest budget column automatically highlighted in emerald with a trophy icon.
            </span>
          </p>
